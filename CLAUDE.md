...
- [ ] 后续:image_pushed 真 webhook(桌面架构冲突,可能永久跳过;手动入口已就绪)+ report 调度持久化优化 + **真集群 GUI smoke**(集群恢复后:验证 code-repo 节点 + CALLS 边 + change 事件渲染;GNOME Wayland 程序化截图受阻,需肉眼)+ **Phase 8.2:C1 Identity Resolver v1**(correlation-key 合并,让 code-repo BUILDS 挂上 k8s image 节点 → repo→image→runtime 联通;见 Phase 6 C1 行)+ code-repo/flagd/k8s-events 真集群 smoke(集群恢复后)
