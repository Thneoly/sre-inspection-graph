...
- [ ] Phase 3 延后:real handler 接入(3.9a-3b2 WasmHandler + WasmHandlerExecutor + 真集群验证;async 基础已就绪)+ k8s-watch connector + webhook(桌面架构冲突,可能跳过)+ RUNS/EXPOSES 边(需 trace/ingress 层)
...
- [ ] PRD-003 后续:report 跨重启历史持久化(ReportStore -> SQLite)+ 真集群 X11 smoke + k8s-watch connector(自动录 change event)
