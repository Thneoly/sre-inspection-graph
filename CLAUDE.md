...
| C1 | **Identity Resolver v1**(correlation-key 合并 + field-ownership 完整表 + 冲突仲裁,doc/11 §4-5)。**DEFERRED → Phase 8.2**:需一个对**已有节点**做异构身份声明的源才有真实跨源合并冲突。trace(Phase 7)只产 CALLS 边挂到已有 comp 节点,不产 novel 节点也无重叠身份声明 → 无冲突。**code-repo(Phase 8.1)的 `BUILDS` 边(repo→ContainerImage)才是真冲突源**:两端对同一 image 用不同 key(k8s `image:{c}:{ns}:{ref}` vs repo 只知 `{ref}`),correlation key = 原始 image-ref 串(k8s 存 attrs verbatim,repo 从 Dockerfile/push 推导)→ C1 的合并靶子。v0 下 BUILDS 因缺 cluster/ns 悬空被 `facts_to_graph` 过滤;8.2 建合并机(+ image-ref normalize + field-ownership)让 BUILDS 挂上 → repo→image→container→pod 联通。CodeRepo/Library 节点本身是新层(同 trace),不触发合并 | ⏸ Phase 8.2 |
...
- [ ] 后续:image_pushed 真 webhook(桌面架构冲突,可能永久跳过;手动入口已就绪)+ report 调度持久化优化 + flagd 真集群 smoke(本集群未部署 flagd,需装)+ **Phase 8.2:C1 Identity Resolver v1**(correlation-key 合并,让 code-repo BUILDS 挂上 k8s image 节点 → repo→image→runtime 联通;见 Phase 6 C1 行)
