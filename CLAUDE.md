...
- [ ] Phase 3 延后:real handler 接入(3.9a-3b2 WasmHandler + WasmHandlerExecutor + 真集群验证;async 基础已就绪)+ webhook(image_pushed;桌面架构冲突,可能跳过)+ RUNS/EXPOSES 边(需 trace/ingress 层)。注:k8s-watch 已以 poll-diff 落地(见 PRD-003/002 后续条目),非 streaming watch
...
- [ ] 后续:image_pushed 真 webhook(桌面架构冲突,需 HTTP server,可能永久跳过;手动录入口已就绪)+ RUNS/EXPOSES 边(需 trace/ingress 层)+ report 调度持久化优化
