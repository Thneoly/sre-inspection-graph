...
import { fetchAccessLink } from '../../api/client';
...
export default function AccessLinkView() {
  const [appCode, setAppCode] = useState('order');
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [isEdge, setIsEdge] = useState(false);
  const [layers] = useState<Set<LayerName>>(defaultLayers());
  ...
  return (
    <div style={{ height: 'calc(100vh - 80px)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <GraphToolbar>
        ...
        <Select value={appCode} onChange={setAppCode} style={{ width: 180 }} options={[
          { value: 'order', label: '订单应用 (order)' },
        ]} />
        <LayerToggle activeLayers={layers} onChange={() => {}} />
      </GraphToolbar>
